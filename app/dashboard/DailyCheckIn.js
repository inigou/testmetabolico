...
const getColorEnergia = (val) => val >= 7 ? C.green : val >= 4 ? '#E8A020' : C.orange;
const getColorSueno = (val) => val >= 7 ? C.green : val >= 4 ? '#E8A020' : C.orange;
const getColorEstres = (val) => val <= 3 ? C.green : val <= 6 ? '#E8A020' : C.orange;

export default function DailyCheckIn({ email, perfil, objetivoId, onWeatherUpdate }) {
  ...
  useEffect(() => {
    ...
    if (cached) {
      ...
      if (data.weather) {
        ...
        if (onWeatherUpdate) onWeatherUpdate(data.weather);
      }
    }
  }, [email]);
  ...
  const guardarCheckIn = async () => {
    ...
    try {
      await fetch(`${SUPABASE_URL}/rest/v1/daily_logs`, {
        ...
        body: JSON.stringify({ email, fecha: hoy(), energia, sueno, estres }),
      });
  ...
  const sliders = [
    { label: 'Energía', value: energia, set: setEnergia, getColor: getColorEnergia, emoji: '⚡', desc: ['Agotado', 'Bajo', 'Normal', 'Activo', 'Excelente'] },
    { label: 'Sueño', value: sueno, set: setSueno, getColor: getColorSueno, emoji: '😴', desc: ['Pésimo', 'Malo', 'Regular', 'Bueno', 'Perfecto'] },
    { label: 'Estrés', value: estres, set: setEstres, getColor: getColorEstres, emoji: '🧠', desc: ['Nulo', 'Leve', 'Moderado', 'Alto', 'Máximo'] },
  ];

  const getDesc = (val, arr) => arr[Math.min(4, Math.floor((val - 1) / 2))];

  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>

      {/* Weather card — aparece tras guardar */}
      {weather && (
        <div style={{
          background: `linear-gradient(135deg, ${C.green}, #3B6D11)`,
          borderRadius: 16, padding: '20px 24px',
          display: 'flex', justifyContent: 'space-between', alignItems: 'center',
        }}>
          <div>
            <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.65)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 6 }}>
              Tu estado metabólico hoy
            </div>
            <div style={{ fontFamily: 'Georgia, serif', fontSize: 22, color: C.white, marginBottom: 8 }}>
              {weather.estado}
            </div>
            <div style={{ fontSize: 13, color: 'rgba(255,255,255,0.9)', lineHeight: 1.5, maxWidth: 340 }}>
              {weather.consejo}
            </div>
          </div>
          <div style={{ fontSize: 48, flexShrink: 0, marginLeft: 12 }}>
            {weather.estado?.split(' ').pop()}
          </div>
        </div>
      )}
        ...
        <div style={{ padding: '20px' }}>
            ...
            <div key={i} style={{ marginBottom: i < sliders.length - 1 ? 20 : 0 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <span style={{ fontSize: 11, color: C.mid }}>{getDesc(s.value, s.desc)}</span>
                  <span style={{ fontFamily: 'Georgia, serif', fontSize: 20, color: s.getColor(s.value), fontWeight: 700, minWidth: 28, textAlign: 'right' }}>{s.value}</span>
                </div>
              </div>
              <div style={{ position: 'relative' }}>
                <input
                  ...
                  style={{ width: '100%', accentColor: s.getColor(s.value), cursor: yaHizoCheckIn ? 'default' : 'pointer' }}
                />
          ...
          {cargandoWeather && (
            <div style={{ textAlign: 'center', marginTop: 12, fontSize: 12, color: C.mid }}>
              Calculando tu Metabolic Weather...
            </div>
          )}
        </div>
      </div>
